...
exports.checkRoles = function (user, role) {
    if (!internals.isGranted(user.role, role)) return Boom.unauthorized('Unauthorized');
    return null;
};
...
exports.fetchEntity = function (query, param, request, cb) {
    ...
    query(param, function (err, entity) {
        if (err) return def.reject(Boom.internal('Bad request', err));
        else if (!entity) return def.reject(Boom.notFound());
    ...
    if (!entity) def.reject(new Error('validateUserACL must run after fetchACLEntity'));
        ...
        if (validator) {
            entity[validator](user, role, function (err, isValid) {
                ...
                // Not granted
                else if (!isValid) def.reject(Boom.unauthorized('Unauthorized'));
            ...
            if (!isValid) def.reject(Boom.unauthorized('Unauthorized'));
            else def.resolve(isValid);
        }
    }
