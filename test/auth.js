...
describe('AuthTokenSchema', function () {
    describe('initialize', function () {
        it('validated the presence of tokenValidator', function (done) {
            ...
            server.connection();
            server.register({register: Plugin, options: {tokenAuth: true}},
                function (err) {
        ...
        it('auth-token strategy is not available when tokenAuth is false', function (done) {
            ...
            server.connection();
            server.register({register: PluginObject, options: {tokenAuth: false}},
                function (err) {
                    expect(function () {
                        server.auth.strategy('default', 'auth-token');
                    }).to.throw(/unknown scheme: auth-token/);
                    done();
                });
        });
        ...
        it('auth-token strategy is available when tokenAuth is enabled', function (done) {
            var server = new Hapi.Server(0);
            server.connection();
            server.register({register: PluginObject, options: {tokenAuth: {tokenValidator: function () {}}}},
                function (err) {
    ...
    describe('#authenticate', function () {
        it('returns error when a token is not found', function (done) {
            var server = new Hapi.Server(0);
            server.connection();
            server.register({register: PluginObject, options: {tokenAuth: {tokenValidator: function (token, cb) {cb(null, null)}}}},
                function (err) {
                    server.auth.strategy('default', 'auth-token');
